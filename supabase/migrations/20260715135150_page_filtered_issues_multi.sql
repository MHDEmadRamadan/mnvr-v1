...
begin
  ...
  with base as (
    ...
    where
      ...
      and (
        p_filters->'pmmSoftware' is null or p_filters->'pmmSoftware' = 'null'::jsonb or (
          case
      ...
      and (coalesce(p_filters->>'imeiChanged', '') = '' or (case when lower(trim(p_filters->>'imeiChanged')) in ('false','no','no change') then lower(coalesce(r.imei_changed,'')) in ('false','no','no change','') when lower(trim(p_filters->>'imeiChanged')) in ('true','yes') then lower(coalesce(r.imei_changed,'')) not in ('false','no','no change','') else coalesce(r.imei_changed,'') ilike '%' || public.escape_ilike_pattern(p_filters->>'imeiChanged') || '%' end))
      and (coalesce(p_filters->>'simChanged', '') = '' or (case when lower(trim(p_filters->>'simChanged')) in ('false','no','no change') then lower(coalesce(r.sim_changed,'')) in ('false','no','no change','') when lower(trim(p_filters->>'simChanged')) in ('true','yes') then lower(coalesce(r.sim_changed,'')) not in ('false','no','no change','') else coalesce(r.sim_changed,'') ilike '%' || public.escape_ilike_pattern(p_filters->>'simChanged') || '%' end))
      ...
      and (v_search_pat is null or i.issue_type ilike v_search_pat or i.motherboard_issue ilike v_search_pat or i.pmm_issue ilike v_search_pat or i.ssd_issue ilike v_search_pat or i.other_issue ilike v_search_pat or coalesce(i.description,'') ilike v_search_pat or coalesce(d.imei,'') ilike v_search_pat or coalesce(d.tickets,'') ilike v_search_pat or coalesce(d.description,'') ilike v_search_pat or coalesce(v.vehicle_number,'') ilike v_search_pat or coalesce(v.description,'') ilike v_search_pat or coalesce(ds.software_version,'') ilike v_search_pat or coalesce(ds.flespi_status,'') ilike v_search_pat or coalesce(ds.screen_status,'') ilike v_search_pat or coalesce(ds.dotmatrix_status,'') ilike v_search_pat or cast(ds.pmm_software as text) ilike v_search_pat or coalesce(h.motherboard_type,'') ilike v_search_pat or coalesce(h.pmm_type,'') ilike v_search_pat or coalesce(s.ssd_type,'') ilike v_search_pat)
      and (not v_critical or i.issue_type ilike '%critical%' or i.issue_type ilike '%urgent%' or i.motherboard_issue ilike '%critical%' or i.motherboard_issue ilike '%fail%' or i.pmm_issue ilike '%critical%' or i.pmm_issue ilike '%fail%' or i.ssd_issue ilike '%critical%' or i.ssd_issue ilike '%fail%')
  ),
  ordered as (
