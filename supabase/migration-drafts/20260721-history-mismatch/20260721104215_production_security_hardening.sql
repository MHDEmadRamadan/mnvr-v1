-- Production security hardening from advisor audit.

-- Drop unused SECURITY DEFINER view (app never queries it).
drop view if exists public.issues_safe;

-- Lock down sensitive RPCs: authenticated only (functions already check auth.uid / is_admin).
...
-- Fix mutable search_path on helpers (keep existing parameter names).
...
-- Covering index for issues.resolved_by FK / Edited-by filters.
create index if not exists issues_resolved_by_idx on public.issues (resolved_by);

-- Optimize profiles RLS init plans.
